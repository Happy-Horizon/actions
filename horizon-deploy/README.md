...
**Build-generated `env.php`:** `setup:di:compile` may write a minimal `app/etc/env.php` (cache types only). Packages that read DB config during Composer/Magento bootstrap (notably `experius/connector-interface-magento`) then fail `setup:static-content:deploy` with *No database connection was found…*. Central `build_tasks` runs `magento:build:stub-env` before `magento:deploy:assets` (dummy `db.connection.default`) so split SCD (adminhtml then frontend) keeps working after Magento rewrites cache types, then `magento:build:remove-env` after SCD. The real `env.php` comes from `shared_files` on the server. If you replace `build_tasks` on a stage/project, keep both tasks around `magento:deploy:assets`.
